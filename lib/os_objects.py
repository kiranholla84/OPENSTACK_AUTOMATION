...
class VolumeOperations(object):
    ...
    def __init__(self, bootable_factor = 'nonbootable', size_vol = 1, type_vol = 'VMAX_DIAMOND', volume_name_prefix = 'test_qe', replication_factor=None, number_of_volumes=5):
    ...
    def volume_type_return(self, replication_type):
        ...
        for self.volume_type_in_new_list in self.new_list:
            self.show_check_output = ['openstack', 'volume', 'type', 'show', self.volume_type_in_new_list, '-f', 'json']
            ...
            # print "DEBUG: volume type is", self.volume_type_in_new_list

            self.match_pattern = re.search(self.pattern, self.input_string, re.I)
            if self.match_pattern:
                self.replication_list.append(self.volume_type_in_new_list)
            else:
                self.non_replication_list.append(self.volume_type_in_new_list)
    ...
    def volumes_create(self):
        ...
        self.type_vol = []
        self.size_vol = []
        self.volume_array_objects = []

        # Execute creation of all volumes in parallel. This is not a multi-threaded way of calling though
        for volume_index in range(0, self.number_of_volumes):

            # Append the latest volume name to the volume name list
            self.volume_name.append((self.volume_name_prefix + str(volume_index)))

            print "\n================CREATING %s VOLUME %s ================\n" % (self.bootable_factor , self.volume_name[volume_index])

            # Append the latest volume size to the volume size list. Currently it will be based on select_volume_sizes. This can be controlled from outside by intake of variables
            self.size_vol.append(self.select_volume_size())

            # Append the volume type to the array
            self.type_vol.append(self.volume_type_return(self.replication_type))
        ...
        for volume_index in range(0, self.number_of_volumes):
            ...
            self.volume_array_objects.append(self.async_task_wait_process_for_volume("volume", self.volume_name[volume_index], self.available_string))
            print "DEBUG:VOLUME OBJECT IS %s" %(self.volume_array_objects[volume_index])
            print "DEBUG:NAME %s" % (self.volume_array_objects[volume_index]['name'])
            print "DEBUG:SIZE %s" % (self.volume_array_objects[volume_index]['size'])

            # Creating a dictionary below to send it as an object to the volume check. Embeddint required items into the dictionary
            self.volume_object_index_dict = {volume_index : self.volume_array_objects[volume_index]}

            # Invoking volume_check for each element of the array
            self.comparison_check_parameter = self.volume_check(self.volume_object_index_dict, 'create')
        ...
        print "\nDEBUG: FINAL VOLUME ARRAY IS %s" %self.volume_array_objects
        print "\nDEBUG: FINAL VOLUME ARRAY IS WITH INDEX COUNT %s" % self.volume_object_index_dict

    def volume_check(self, volume_object_index_dict , type_of_operation, *args):

        self.type_of_operation = type_of_operation
        self.object_index_dict = volume_object_index_dict
        ...
        if (self.type_of_operation == 'create'):
            ...
            self.inputs = [self.volume_name[self.object_index_dict.keys()[0]], self.size_vol[self.object_index_dict.keys()[0]], self.type_vol[self.object_index_dict.keys()[0]], self.available_string, VolumeOperations.bootable_string]
        ...
        if (self.type_of_operation == 'extend'):
            ...
            print "DEBUG3 : is %s" % self.volume_array_objects[self.object_index_dict.keys()[0]]
            print "DEBUG3a : is %s" % (self.volume_array_objects[self.object_index_dict.keys()[0]])['size']
            print "DEBUG3b : is %s" % self.extension_factor
            self.intended_extended_size = (self.volume_array_objects[self.object_index_dict.keys()[0]])['size'] + self.extension_factor
            print "DEBUG4 : is %s" %self.intended_extended_size
            self.inputs = [(self.volume_array_objects[self.object_index_dict.keys()[0]])['name'] , self.intended_extended_size]
    ...
    def volume_extend(self, volume_name_prefix, extension_factor = 100):

        self.extension_factor = extension_factor
        self.length_volume_array = len(self.volume_array_objects)
        self.extended_volume_details = list()
        self.extended_volume_object_index_dict = dict()
        print "\nDEBUG: LENGTH OF THE VOLUME ARRAY %s" %self.length_volume_array

        for self.volume_index in range(0, self.length_volume_array):

            # print "\n================VOLUME EXTEND================\n"
            self.intended_extended_size = self.volume_array_objects[self.volume_index]['size']  + self.extension_factor

            self.list_check_output = ['openstack' , 'volume' , 'set' , '--size' , str(self.intended_extended_size), self.volume_array_objects[self.volume_index]['name']]
            self.op = subprocess.check_output(self.list_check_output)

            # Async task, call relevant procedure
            self.extended_volume_details.append(self.async_task_wait_process_for_volume("volume", self.volume_array_objects[self.volume_index]['name'],self.available_string))

            print "NEW EXTENDED SIZE OF VOLUME %s IS %s" %(self.extended_volume_details[self.volume_index]['name'], self.extended_volume_details[self.volume_index]['size'])

            # Creating a dictionary below to send it as an object to the volume check. Embeddint required items into the dictionary
            self.extended_volume_object_index_dict = {self.volume_index : self.extended_volume_details[self.volume_index]}

            # Check for the volume creation
            self.comparison_check_parameter = self.volume_check(self.extended_volume_object_index_dict, 'extend', self.extension_factor)
            if self.comparison_check_parameter == 0:
                print "VOLUME %s SUCCESSFULLY EXTENDED" % (self.extended_volume_details[self.volume_index]['name'])
            else:
                print "VOLUME NOT CREATED"
